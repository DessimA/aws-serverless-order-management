...
# 2. IAM Role & Permissions (Adicionado EventBridge)
...
aws iam put-role-policy --role-name "$ROLE_NAME" --policy-name "BatchProcessorPolicy" --policy-document "{
    \"Version\": \"2012-10-17\",
    \"Statement\": [
        ...
        {\"Effect\": \"Allow\", \"Action\": [\"sqs:ReceiveMessage\",\"sqs:DeleteMessage\",\"sqs:GetQueueAttributes\"], \"Resource\": \"$SQS_S3_ARN\"}
    ]
}"
...
if ! aws lambda get-function --function-name "$LAMBDA_NAME" --region "$REGION" >/dev/null 2>&1; then
    aws lambda create-function --function-name "$LAMBDA_NAME" --runtime python3.12 --role "arn:aws:iam::$ACCOUNT_ID:role/$ROLE_NAME" --handler index.lambda_handler --zip-file fileb://batch_lambda.zip --region "$REGION" --timeout 60
...
aws lambda update-function-configuration --function-name "$LAMBDA_NAME" --region "$REGION" \
    --environment "Variables={DYNAMODB_TABLE=$TABLE_NAME,SNS_TOPIC_ARN=$SNS_ARN,EVENT_BUS_NAME=$BUS_NAME}"
