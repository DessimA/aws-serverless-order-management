...
# 1. IAM Role com permissoes completas (SQS e DynamoDB)
if ! aws iam get-role --role-name "$ROLE_NAME" >/dev/null 2>&1; then
    aws iam create-role --role-name "$ROLE_NAME" --assume-role-policy-document '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"Service":"lambda.amazonaws.com"},"Action":"sts:AssumeRole"}]}'
fi
aws iam attach-role-policy --role-name "$ROLE_NAME" --policy-arn arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole

# 2. Provisionamento de Filas e Regras (Loop para otimizacao)
OPS=("cancel" "update")
for OP in "${OPS[@]}"; do
    ...
    # Trigger SQS -> Lambda
    MAPPING=$(aws lambda list-event-source-mappings --function-name "$LAMBDA" --region "$REGION" --query "EventSourceMappings[?EventSourceArn=='$SQS_ARN'].UUID" --output text)
...
echo "--- Lifecycle Operations Deployed ---"