...
Consome a fila SQS FIFO de validação, publica pedidos validados no EventBridge e dispara alertas SNS em caso de falha.

## Comportamento

1. Le cada mensagem da fila SQS FIFO.
2. Se o registro não possui `pedidoId` ou `clienteId` (campo ausente):
   - Publica alerta SNS com o conteúdo do record.
   - Nao adiciona o `messageId` a `batchItemFailures` (reenvio não resolve payload malformado).
   - Continua para o próximo record.
